...
def main():
    mwr = RadarObsFile("qc/1km/vols/05XP.20090605.215719")
    ...
    pylab.colorbar()
    pylab.contour(xs, ys, mwr['vr'][0][bounds], levels=np.arange(-40, 0, 5), linestyle='--', colors='k')
    pylab.contour(xs, ys, mwr['vr'][0][bounds], levels=[0], linestyle='-', linewidths=1.5, colors='k')
    pylab.contour(xs, ys, mwr['vr'][0][bounds], levels=np.arange(5, 45, 5), linestyle='-', colors='k')

...
class DataIO:
    ...
    def __init__(self, file_name=None, mode=None, format='nc'):
        """
        __init__()
        ...
        if file_name is not None:
            self.open_file(file_name, mode, format)
        return
...
#           self._write = False
    ...
    def get_file_attribute(self, attribute):
        ...
        try:
            if reader.__name__ == "Dataset":
                return self._df.ncattrs[attribute]
            else:    
    ...
    def get_file_attribute_list(self):
        ...
        """
        return [ a for a in dir(self._df) if a[0] != '_' ]
    ...
    def close(self,out_file_name=''):
        ...
        if self._write:
            ...
            tmp_file = None
            if reader.__name__ == "Nio":
                tmp_file = reader.open_file(tmp_file_name, mode='w', format=self._format)
            elif reader.__name__ == "scipy.io.netcdf":
            ...
            for attribute in dir(self._df):
                if attribute[:2] != "__" and not hasattr(tmp_file, attribute) and attribute not in self._rm_attributes['__file__']:
...
#   p = nc.get_variable('P')
    ...
    print nc.get_variable_attribute('P', 'units')
